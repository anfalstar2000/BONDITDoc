...
export function MobileSearchBar({ onNavigate }: MobileSearchBarProps) {
 ...
 className="w-full pr-10 pl-10 py-2 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[#3B82F6] focus:ring-offset-2 focus:border-[#3B82F6] rounded-none transition-colors"
 />
 {query && (
 <button
 onClick={() => setQuery('')}
 ...
 {results.length > 0 && (
 <div className="mt-2 bg-gray-50 rounded-3xl border-gray-200">
 {results.map((result, index) => (
 <button
 key={index}
 onClick={() => handleSelect(result.path)}
 className="w-full text-right px-3 py-2 hover:bg-white transition-colors border-b border-gray-200 last:border-b-0"
 >
